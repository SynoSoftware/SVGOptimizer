...
function buildConfig(options?: GeometryOptimizationOptions): OptimizerConfig {
  return {
    ...DEFAULT_CONFIG,
    ...options,
    // Clamp obvious numeric values to keep the engine stable
...
type ImportedShape = {
  ...
  originalD: string;
  isSolid: boolean; // New flag for masking logic
  strokeWidth?: number;
};
type ProcessedShape = {
  pathData: string;
...
/**
 * Full pipeline: geometry-first, optional raster occlusion refinement.
 */
  ...
  try {
    ...
    for (let i = 0; i < shapeElements.length; i++) {
      ...
      try {
        ...
        if (opacity < 0.99) {
           attrs.unique['opacity'] = opacity.toFixed(3);
           if(attrs.unique['fill-opacity']) delete attrs.unique['fill-opacity'];
           if(attrs.inheritable['fill-opacity']) delete attrs.inheritable['fill-opacity'];
        }
    ...
    // --- BOOLEAN PROCESSING PHASE (Top -> Bottom) ---
    for (let i = totalLayers - 1; i >= 0; i--) {
      ...
      // Trap Logic: Only trap if solid AND we cut it.
      ...
      // 6. Update occluders collection using the final visible geometry
...
/**
 * Cleans up a boolean operation result. Removes tiny islands (artifacts) that
 * are often created by floating point errors.
 */
function cleanBooleanResult(
  ...
  if (item.className === "CompoundPath") {
    ...
    for (let i = children.length - 1; i >= 0; i--) {
      const child = children[i] as paper.Path;
...
function expandStrokeOutline(
  ...
  try {
    ...
    // ignore failing stroke expansions
  }
  clone.remove();
  return null;
}

// ===================== OPACITY & SOLIDITY HELPERS =====================

/**
 * Traverses up the DOM to calculate the true effective opacity of an element.
 * Returns the scalar opacity and a boolean indicating if it counts as a "solid occluder".
 */
function getComputedOpacityAndSolidity(el: Element): {
  ...
  if (fill.startsWith("#") && fill.length === 9) {
    if (parseInt(fill.slice(7), 16) < 255)
      return { opacity: 1, isSolid: false, strokeOccluder: false };
  }
  ...
  while (current && current.tagName !== "svg") {
    ...
    const blend = current.getAttribute("mix-blend-mode");
    if ((blend && blend !== "normal") || style.includes("mix-blend-mode")) {
      return { opacity: totalOpacity, isSolid: false, strokeOccluder: false };
    }
...
function importToPaper(
  ...
  if (typeof (item as any).resolveCrossReferences === "function") {
    try {
      ...
      // Ignore cross reference errors and continue with raw geometry
    }
  }
